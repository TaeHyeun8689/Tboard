						...
						<tbody>
							<c:forEach items="${list}" var="list">
								<tr>
									...
									<td>
										<a href="board/view${pageMaker.makeQuery(pageMaker.pc.page)}&board_num=${list.board_num}">${list.board_title}</a>
										<c:if test="${list.board_reply_count > 0}">
											[${list.board_reply_count}]
										</c:if>
									</td>
									<td>${list.board_writer}</td>
									<td>
										<fmt:formatDate pattern="yyyy-MM-dd" value="${list.board_reg_date}" />
									</td>
